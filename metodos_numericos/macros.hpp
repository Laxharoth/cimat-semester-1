...
#define matrix_to_ptr(matrix, ptr, size)                                       \
...
template <typename T> void print_array(T &array, size_t size) {
  ...
  for (size_t i = 0; i < size; ++i) {
    (*(macros::out)) << std::setfill(' ') << std::setw(7)
                     << std::setprecision(2) << array[i] << " ";
  }
  (*(macros::out)) << std::endl;
}
template <typename T> void print_matrix(T &matrix, size_t size) {
  for (size_t i = 0; i < size; ++i) {
    auto a = matrix[i];
    print_array(a, size);
  }
