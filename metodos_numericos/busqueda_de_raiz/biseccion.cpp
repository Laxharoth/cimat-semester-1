...
double biseccion(FunctionWrapper *funcion, double x_inferior, double x_superior,
                 int max_iter, int *iteraciones, double *error) {
  ...
  while ((*real_iter) < max_iter) {
    ...
    if (std::abs(y_medio) <= TOLERANCIA)
      return x_medio;
    if (y_medio * y_inferior > 0) {
      if (error != nullptr)
        (*error) = calc_err(x_inferior, x_medio);
      x_inferior = x_medio;
    } else {
      if (error != nullptr)
        (*error) = calc_err(x_superior, x_medio);
      x_superior = x_medio;
    }
  }
  return x_medio;
