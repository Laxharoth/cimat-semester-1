...
std::unique_ptr<std::vector<min_max_value>> search_min_max(FunctionWrapper *fw,const double &x_inferior,const double &x_superior){
    std::unique_ptr<std::vector<min_max_value>> ptr_search_min_max = std::make_unique<std::vector<min_max_value>>();
    std::vector<min_max_value> &puntos = *ptr_search_min_max;

    Derivative derivada(fw);
    Derivative derivada_2(&derivada);

    for(double x_izquierda = x_inferior; x_izquierda < x_superior; x_izquierda += SEARCH_MIN_MAX_DELTA ){
