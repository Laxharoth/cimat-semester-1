...
vector::vector(std::initializer_list<double> initial):size(initial.size()),allocated(true){
    this->data = new double[this->size];
    size_t i{0};
    for(auto j= initial.begin(); j!= initial.end(); ++j,++i)
        data[i] = *j;
}
vector::vector(double *data, size_t size):size(size),allocated(false),data(data){}
vector::~vector(){
...
struct vector_coef_data{size_t begin; size_t end; vector *v;double coef;};
...
struct vector_vector_data{size_t begin; size_t end; const vector* const v1; const vector* const v2; double * const sum;};
...
struct vector_sum_vector_data{size_t begin; size_t end; vector* const v1; const vector* const v2;};
...
struct vector_cross_vector_data{size_t begin; size_t end; const vector* const v1; const vector* const v2; matrix *res;};
...
matrix::Column::Column(const matrix::Column &other):size(other.size),increment(other.increment),data(other.data){}
...
struct column_sum_vector_data{size_t begin; size_t end; matrix::Column* const v1; const vector* const v2;};
matrix::Column &matrix::Column::operator-=(const vector &other){
    #ifndef NO_ASYNC
    if(size > MIN_OPER_FOR_THREAD){
    std::vector<std::future<void>> futures;
...
double reduce(const vector &v,double (*callback)(double acc, const double cur),const double start){
    double acc = start;
