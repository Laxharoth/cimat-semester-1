...
auto phi_i_j= [](const int i, const int j){ return std::sin(i*j*h*PI); };
...
int main(int argc, char const *argv[])
{
    ...
    strm_out("value");
    strm_out( std::abs(std::abs(val) - std::abs(biggest_l)) )
    strm_out("vector");
    int i = 0;
    double error = 0;
    double contrl= 0;
    ...
    strm_out("value");
    strm_out( std::abs(std::abs(val) - std::abs(smallest)) )
    strm_out("vector");
    error = 0;
    contrl= 0;
