...
/**
 * @brief Mapea los puntos en el intervalo especificado y actualiza los minimos
 * y maximos de x y f(x)
 */
void init_points(GtkWidget *graph, double (*single_variable_func)(double),
...
gint main(int argc, char *argv[]) {
  ...
  radio_original =
      GTK_WIDGET(gtk_builder_get_object(builder, "radio_original"));
  radio_derivada =
      GTK_WIDGET(gtk_builder_get_object(builder, "radio_derivada"));
  radio_derivada_2 =
      GTK_WIDGET(gtk_builder_get_object(builder, "radio_derivada_2"));
  ...
  g_signal_connect(radio_original, "toggled",
                   G_CALLBACK(on_radio_original_toggle), (gpointer) "1");
  g_signal_connect(radio_derivada, "toggled",
                   G_CALLBACK(on_radio_original_toggle), (gpointer) "2");
  g_signal_connect(radio_derivada_2, "toggled",
                   G_CALLBACK(on_radio_original_toggle), (gpointer) "3");
...
void draw_axis(cairo_t *cr, const gint width, const gint height,
  ...
  const gint distance_width = width / axis_lines,
             distance_height = height / axis_lines;
  const double middle_x = (max_x + min_x) / 2, middle_y = (max_y + min_y) / 2;
  const double change_x = (max_x - min_x) / axis_lines,
               change_y = (max_y - min_y) / axis_lines;
  cairo_set_line_width(cr, 0.5);
  if (middle_x == 0 && middle_y == 0)
    cairo_set_line_width(cr, 1.5);
  cairo_set_source_rgb(cr, 0.0, 0.0, 0.0);
  cairo_move_to(cr, 0, middle_height);
  cairo_line_to(cr, width, middle_height);
  cairo_move_to(cr, middle_width, 0);
  cairo_line_to(cr, middle_width, height);
  for (gint line = 0; line < axis_lines / 2; ++line) {
    ...
    cairo_move_to(cr, middle_width + distance_width * line + 4,
                  middle_height + 10);
    cairo_show_text(cr,
                    trim_double_to_str(middle_x + change_x * line, 2).c_str());
    // up
    ...
    cairo_move_to(cr, middle_width + 4,
                  middle_height - distance_height * line - 10);
    cairo_show_text(cr,
                    trim_double_to_str(middle_y + change_y * line, 2).c_str());
    if (line == 0)
      continue;
    // left
    cairo_move_to(cr, middle_width - distance_width * line, middle_height - 4);
...
void init_points(GtkWidget *graph, double (*single_variable_func)(double),
  ...
  analize_single_var_function(
      [](double x) { return current_function->eval(x); }, min_x, max_x,
      drawer.min_value.y, drawer.max_value.y, points);
...
gboolean on_graph_draw(GtkWidget *graph, cairo_t *cr, gpointer data) {
  ...
  drawer.scale_x = canvas_facade::choose_scale_helper(
      drawer.min_value.x, drawer.max_value.x, width);
  drawer.scale_y = canvas_facade::choose_scale_helper(
      drawer.min_value.y, drawer.max_value.y, height);
  drawer.set_origin(canvas_facade::choose_origin_helper(
      drawer.min_value.x, drawer.min_value.y, drawer.scale_x, drawer.scale_y,
      height));
  drawer.draw_multiline((void *)cr, points.data(), points.size());
  draw_axis(cr, width, height, drawer.min_value.x, drawer.max_value.x,
            drawer.min_value.y, drawer.max_value.y);
  return FALSE;
}
void on_btn_graph_click(GtkWidget *btn, gpointer data) {
...
void on_radio_original_toggle(GtkWidget *widget, gpointer data) {
  ...
  if (data == "2") {
    current_function = &derivative;
  }
  if (data == "3") {
    current_function = &derivative_2;
  }
  on_btn_graph_click(btn_graph, NULL);
}
std::string trim_double_to_str(const double &num, const int &precision) {
  ...
  int cut = n_to_s.find(".");
  if (cut < 0 || cut + precision + 1 > n_to_s.length())
    return n_to_s;
