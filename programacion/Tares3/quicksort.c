...
void quicksort(void *data, int start, int end, unsigned int data_size,
  ...
  int mid = right;
  while (1) {
    while (cmp(data + left * data_size, data + mid * data_size) < 0)
      left++;
    while (right > 0 &&
           cmp(data + right * data_size, data + mid * data_size) > 0)
      right--;
    if (left >= right)
      break;
