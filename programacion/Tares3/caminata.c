...
void set_at( char new_char, char *grid ,grid_meta grid_dim, unsigned int y, unsigned int x);
bool is_free_at(char *grid ,grid_meta grid_dim, unsigned int y, unsigned int x);
bool can_move_from(char *grid ,grid_meta grid_dim, unsigned int y, unsigned int x);
...
int main(int argc, char const *argv[]){
    ...
    char grid[100];
    for(int i=0; i<100; i++)grid[i]='.';
    grid_meta meta;
    meta.height = 10;meta.width=10;
    char current_char = '.';
    position current_pos;
    position new_pos;
    current_pos.y = 0; current_pos.x = 0;
    // initial position
    grid[0] = ++current_char;
    while(1){
        if(! can_move_from(grid, meta, current_pos.y, current_pos.x) ) break;
        do{
            new_pos = current_pos;
...
char get_at( char *grid ,grid_meta grid_dim, unsigned int y, unsigned int x){
    if( x < 0 || x >= grid_dim.width ||
        y < 0 || y >= grid_dim.height){ return 0;}
    return grid[ y*grid_dim.width + x ];
}
void set_at( char new_char, char *grid ,grid_meta grid_dim, unsigned int y, unsigned int x){
    if( x < 0 || x >= grid_dim.width ||
        y < 0 || y >= grid_dim.height){ return;}
...
bool is_free_at(char *grid ,grid_meta grid_dim, unsigned int y, unsigned int x){
    if(get_at(grid,grid_dim,y,x) == '.')return true;
    return false;
}
bool can_move_from(char *grid ,grid_meta grid_dim, unsigned int y, unsigned int x){
    return 
