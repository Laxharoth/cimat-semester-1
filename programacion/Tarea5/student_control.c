...
int main(int argc, char const *argv[]) {
  ...
  int eof = 0;
  while (1) {
    handle_statement(eof = fscanf(file, "%s %d %s %c %c", name_buffer,
                                  &age_buffer, grade_buffer, &group_buffer,
                                  &turn_buffer));
    if (eof == EOF)
      break;
    inserted++;
    ...
    stds[inserted - 1] = create_student(name_buffer, age_buffer, grade_buffer,
                                        group_buffer, turn_buffer);
  }
  fclose(file);
  int choice = 0;
  while (choice != '9') {
    ...
    switch (choice) {
    case '1':
      print_students(stds, inserted);
      break;
    case '2':
      print_by_cmp(stds, inserted, cmp_name);
      break;
    case '3':
      print_by_cmp(stds, inserted, cmp_age);
      break;
    case '4':
      print_by_cmp(stds, inserted, cmp_grade);
      break;
    case '5':
      print_count_group(stds, inserted);
      break;
    case '6':
      print_count_turn(stds, inserted);
      break;
    case '7':
      remove_estudiante(&stds, &inserted);
      break;
    case '8':
      add_estudiante(&stds, &inserted, &max_size);
      break;
    case '9':
      save_by_name(stds, inserted);
      break;
    default:
      printf("Selection %c (%d) incorrect", choice, choice);
      getchar();
      break;
    }
  }
...
Estudiante create_student(char *nombre, int edad, char *calif_promedio,
                          char grupo, char turno) {
...
void print_student(Estudiante *student) {
  printf("Nombre:%s\n", student->nombre);
  printf("Edad:%d\n", student->edad);
  printf("Promedio:%c%c\n", student->E->calif_promedio[0],
         student->E->calif_promedio[1]);
  printf("Grupo:%c\n", student->E->grupo);
  printf("Turno:%c\n", student->E->turno);
}
...
void print_by_cmp(Estudiante *student, int count,
                  int (*cmp)(void *a, void *b)) {
  Estudiante studentcpy[count];
  for (size_t i = 0; i < count; i++)
    studentcpy[i] = student[i];
...
int grade_to_int(char grade[2]) {
  ...
  if (grade[0] == 'D' && grade[1] == '-')
    return 6;
  if (grade[0] == 'E' && grade[1] == '+')
    return 5;
  if (grade[0] == 'E' && grade[1] == 0)
    return 4;
  if (grade[0] == 'E' && grade[1] == '-')
    return 3;
  if (grade[0] == 'F' && grade[1] == '+')
    return 2;
  if (grade[0] == 'F' && grade[1] == 0)
    return 1;
  if (grade[0] == 'F' && grade[1] == '-')
    return 0;
  return -1;
...
int cmp_name(void *a, void *b) {
  handle_statement(
      int res = strcmp(((Estudiante *)a)->nombre, ((Estudiante *)b)->nombre));
  return res;
}
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (st->E->calif_promedio[1] == 0)
      fprintf(fp, "%s %d %c %c %c\n", st->nombre, st->edad,
              st->E->calif_promedio[0], st->E->grupo, st->E->turno);
    else
      fprintf(fp, "%s %d %c%c %c %c\n", st->nombre, st->edad,
              st->E->calif_promedio[0], st->E->calif_promedio[1], st->E->grupo,
              st->E->turno);
  }
  fclose(fp);
}
...
void add_estudiante(Estudiante **student, int *count, int *max_size) {
  ...
  char name_buffer[100], grade_buffer[3], group_buffer, turn_buffer;
  int age_buffer;
  handle_statement(scanf("%s %d %s %c %c", name_buffer, &age_buffer,
                         grade_buffer, &group_buffer, &turn_buffer));
  flush_buffer();
  (*count)++;
  if (*count > *max_size) {
    handle_statement(*student =
                         realloc(*student, (*count) * sizeof(Estudiante)));
    *max_size = *count;
  }
  (*student)[(*count) - 1] = create_student(
      name_buffer, age_buffer, grade_buffer, group_buffer, turn_buffer);
}
void flush_buffer() {
  char c = '1';
  while (c != EOF && c != '\n')
