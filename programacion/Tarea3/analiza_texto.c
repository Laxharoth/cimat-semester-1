...
void handle_my_file(const char *filename, unsigned int *filesize,
                    unsigned int *n_lines, unsigned int *char_length,
                    int *abcedary_count, int *abcedary_precedence_count);
void print_letter_percent(int *abcedary_count);
void print_precedende_letter_percent(int *abcedary_precedence_count,
...
int main(int argc, char const *argv[]) {
  ...
  handle_my_file(filename, &filesize, &char_length, &n_lines, abcedary_count,
                 abcedary_precedence_count);
  printf("Filename: %s\n", filename);
  printf("filesize: %d\n", filesize);
  ...
  quicksort(positions, 0, ABC_SIZE, sizeof(letter_position), cmp_letter_count,
            swap_letter_count);
...
void print_precedende_letter_percent(int *abcedary_precedence_count,
  ...
  for (unsigned int i = ABC_SIZE - 1; i >= ABC_SIZE - 10; --i) {
    ...
    for (unsigned int j = 0; j < ABC_SIZE; ++j) {
      printf(
          "\t\t-%c: %lf \n", 'a' + j,
          (double)(abcedary_precedence_count[position[i].position * ABC_SIZE +
                                             j]) /
