...
int main(int argc, char const *argv[]){
    ...
    for (int i = 0; i < 4; i++){
        for( int j = 0; j < 5 ; j++){
            long time = (long)GetTimeStamp( fork_replace( img, rows,cols,median, window_size[i], forks[j]); );
            printf("window:%d, forks:%d, time:%lf\n", window_size[i], forks[j], (double)time*(1e-6));
        }
    }
    #endif // DEBUG
    free(img);
    ...
    if(pid == 0){
        ...
        switch (forks){
            case 2:
                ...
                start_y = child_id*(dim_y / 2); end_y = (child_id+1)*(dim_y / 2);
                break;
            case 3:
                start_x = 0; end_x = dim_x;
                start_y = child_id*(dim_y / 3); end_y = (child_id+1)*(dim_y / 3);
                break;
            case 4:
                start_x = (child_id%2)*(dim_x / 2);end_x = (child_id%2 +1)*(dim_x / 2);
                start_y = (child_id/2)*(dim_y / 2);end_y = (child_id/2 +1)*(dim_y / 2);
                break;
            case 9:
                start_x = (child_id%3)*(dim_x / 3);end_x = (child_id%3 +1)*(dim_x / 3);
                start_y = (child_id/3)*(dim_y / 3);end_y = (child_id/3 +1)*(dim_y / 3);
                break;
            default:
