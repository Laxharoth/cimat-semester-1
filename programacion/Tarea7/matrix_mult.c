...
double *omp_mutl(double *A, double *B, shape shape_A, shape shape_B,
                 shape *shape_C);
double *fork_mutl(double *A, double *B, shape shape_A, shape shape_B,
                  shape *shape_C);
...
int main() {
  ...
  for (size_t i = 0; i < REPETITIONS; i++) {
    time += GetTimeStamp(C = omp_mutl(A, B, shape_A, shape_B, &shape_C););
  }
  printf("Open MP time: %ld microseconds\n", time / REPETITIONS);
  free(C);
  time = 0;
  for (size_t i = 0; i < REPETITIONS; i++) {
    time += GetTimeStamp(C = fork_mutl(A, B, shape_A, shape_B, &shape_C););
  }
  printf("Open MP time: %ld microseconds\n", time / REPETITIONS);
  ...
  for (size_t i = 0; i < shape_A.shape_y; i++) {
    for (size_t k = 0; k < shape_B.shape_y; ++k) {
      for (size_t j = 0; j < shape_B.shape_x; ++j) {
        C[i * shape_C->shape_x + j] +=
            A[i * shape_A.shape_x + k] * B[k * shape_B.shape_x + j];
      }
    }
  }
  return C;
...
#define RELEASE()                                                              \
  ...
  int shmid, pid = 0;
  double *shtC;
  if ((shmid = shmget(key, sizeof(double) * shape_C->shape_y * shape_C->shape_x,
                      IPC_CREAT | 0666)) < 0) {
    RELEASE();
    print_error();
    exit(ERROR_SHMGET);
