...
void fill_table(TreeNode *node, byte_code** current, char *code, unsigned int bit);
unsigned int search_in_table_by_value(const byte_code *table, const byte value);
unsigned int search_in_table_by_code(const byte_code *table, const char *code, unsigned int table_size);
int compress(const byte* file_buffer, unsigned long size,const char *output_file){
    ...
    for (size_t i = 0; i < size; i++){
        ...
        for(unsigned int j = 0; j < pushing_bits_size; ++j){
            ...
            if(current_code[j]=='1')output_buffer[current_byte]|=((unsigned char)0b00000001)<<next_bit_position;
            //make sure ther is a 0 in that position
            if(current_code[j]=='0')output_buffer[current_byte]&=(~(((unsigned char)0b00000001)<<next_bit_position));
            bit++;
        }
    }
    FILE *f = fopen(output_file,"wb");
    write_bytes = bit/8;
    if(bit % 8 != 0) ++write_bytes;
    /* HEADER */
    //original size
    ...
    fclose(f);
    free(output_buffer);
    for (size_t i = 0; i < 0; i++) free(table[i].code);
    delete_Tree(tree);
    free(positions);
    free(abcedary);
    return 0;
...
int decompress(const byte* file_buffer, const char *output_file){
    ...
    for(unsigned long current_bit = 0; current_bit < total_bits; ++current_bit){
        ...
        current_code[current_code_size] = '\0';
        if((table_index = search_in_table_by_code(table, current_code, table_entries))>=0){
...
byte_position *get_sorted_positions(unsigned int *abcedary){
    ...
    quicksort( positions, 0, BITS_COMB, sizeof(byte_position), cmp_byte_count, swap_byte_count );
    return positions;
}
Tree *gen_tree(byte_position *positions, unsigned int *leafs){
    *leafs = BITS_COMB;
    ...
    while (positions[start].count == 0){ start++; (*leafs)--; };
...
unsigned int search_in_table_by_value(const byte_code *table, const byte val){
    ...
    while(val!=table[index].value) ++index;
    return index;
}
unsigned int search_in_table_by_code(const byte_code *table, const char *code, unsigned int table_size){
    for (size_t i = 0; i < table_size; i++){
        if(strcmp(table[i].code, code) == 0) return i;
    }
    return -1;   
}
